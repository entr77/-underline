...
import { PageNumberAnalyzer } from "@/lib/vision/analyzers/page-number-analyzer";
...
export async function POST(request: Request) {
  ...
  try {
    const analyzer = new PageNumberAnalyzer(getAnthropicClient());
    const result = await analyzer.analyze(parseImageInput(imageBase64), context);
    return NextResponse.json(result);
  } catch (error) {
    return NextResponse.json(
      { error: "페이지 번호 감지 실패", detail: String(error) },
      { status: 502 }
    );
  }
}
