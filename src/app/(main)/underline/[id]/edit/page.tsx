...
export default async function EditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: { user } }, { data }] = await Promise.all([
    ...
    (supabase as any).from("underlines").select("id, content, page_number, card_style, book_display, card_bg, card_bg_url, image_url, user_id, book:books(title)").eq("id", id).single(),
  ]);
