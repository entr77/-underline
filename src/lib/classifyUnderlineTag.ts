...
const PROMPT = (contents: string[], book?: { title: string; author: string }) => `다음 밑줄 문장들을 각각 아래 태그 중 해당하는 것으로 분류하세요. 1~3개 선택 가능.
...
export async function classifyUnderlineTags(
  contents: string[],
  ...
  try {
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 256,
        messages: [{ role: "user", content: PROMPT(contents, book) }],
      }),
    });
