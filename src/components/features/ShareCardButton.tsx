...
export default function ShareCardButton({ underline }: Props) {
  ...
  // 이미지 URL을 프록시로 교체해서 CORS 우회
  const proxiedUnderline: Underline = {
    ...underline,
    image_url: proxied(underline.image_url),
    card_bg_url: proxied(underline.card_bg_url),
    book: {
      ...underline.book,
      cover_url: proxied(underline.book.cover_url),
    },
  }
    ...
    <>
      ...
      {open && (
        <>
          ...
          {/* 캡처 대상: 뷰포트 오른쪽 밖에 렌더, html-to-image가 캡처 */}
          <div
            ref={cardRef}
            style={{
              position: 'fixed',
          ...
          >
            <UnderlineCard underline={proxiedUnderline} preview />
          </div>
