  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm font-medium text-[var(--color-ink)]">영역 선택</span>
        <div className="flex gap-1.5">
          <button
            onClick={() => handleRotate("ccw")}
            className="w-9 h-9 rounded-full bg-[var(--color-cream-dark)] flex items-center justify-center hover:bg-[var(--color-border)] transition-colors"
      ...
      >
        ...
        {cropRect ? (
          ...
          <div className="absolute inset-0 flex items-end justify-center pb-4 pointer-events-none">
            <span className="text-white/75 text-xs bg-black/40 rounded-full px-3 py-1.5 backdrop-blur-sm">
              드래그해서 영역 선택 · 건너뛰어도 돼요
            </span>
          </div>
        )}
      </div>
      ...
      <div className="flex gap-3">
        ...
        >
          {isProcessing ? "처리 중..." : cropRect ? "이 부분 분석하기" : "전체 분석하기"}
        </button>
      </div>
    </div>
  );
