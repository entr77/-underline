...
5. tech-stack.md Vision 분석 아키텍처 섹션 업데이트
6. 커밋 & 푸시
```
...
`user_taste_profiles` 테이블은 트리거로 자동 관리된다. 직접 INSERT/UPDATE 금지.

### 집계 방식
- **내 밑줄 태그** → `+1.0점`
- **좋아요한 밑줄 태그** → `+1.5점` (더 강한 취향 시그널)
- **장르**: 내 밑줄의 `books.genre` 기반 카운트
- **표시 조건**: `total_underlines + total_likes >= 5` (데이터 부족 시 숨김)

### 프로필 페이지 표시
- 수동 태그 (`users.tags`): `TagBadge` 컴포넌트 — 사용자가 직접 선택한 취향
- 행동 기반 태그 (`tag_scores`): `TasteProfile` 컴포넌트 — 자동 집계, 라벨 "자주 멈추는 주제"
- 두 영역은 시각적으로 구분됨 (TasteProfile은 forest 10% tint 배지)

### RLS
- SELECT: 누구나 조회 가능 (공개 프로필 표시 목적)
- INSERT/UPDATE: 트리거(`security definer`)만 허용 — 애플리케이션 코드에서 직접 쓰기 금지
