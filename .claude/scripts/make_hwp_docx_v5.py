"""
서식 2 구조로 사업계획서 DOCX 생성 v5
— 다이어그램 4종(아키텍처/간트/퍼널/KPI) + 실제 서비스 스크린샷 삽입
"""
...
SHOT = r'd:\dev\underline\grant-assets'
...
def add_image(path, width_cm=15.5, caption=''):
    if os.path.exists(path):
        ...
        add_body(f'[이미지 파일 없음: {path}]', italic=True)

def add_image_row(items, width_cm=5.0, target_height_cm=None):
    """items: [(path, caption), ...] 가로로 나란히 배치, 표 없이 셀 테두리 숨김
    target_height_cm 지정 시 각 이미지의 실제 가로세로비에 맞춰 폭을 계산해
    행 안의 이미지 높이를 통일한다 (width_cm은 셀 폭으로만 사용, fallback)."""
    ...
    for i, (path, caption) in enumerate(items):
        img_cell = table.rows[0].cells[i]
        img_cell.width = Cm(width_cm)
        ip = img_cell.paragraphs[0]
        ip.alignment = WD_ALIGN_PARAGRAPH.CENTER
        if os.path.exists(path):
            if target_height_cm:
                iw, ih = PILImage.open(path).size
                w_cm = target_height_cm * (iw / ih)
            else:
                w_cm = width_cm - 0.3
            ip.add_run().add_picture(path, width=Cm(w_cm))
        else:
            ip.add_run(f'[이미지 없음: {os.path.basename(path)}]')
...
add_image_row([
    (os.path.join(SHOT, 'shot_home.png'), '① 서비스 첫 화면 — 핵심 메시지'),
    (os.path.join(SHOT, 'shot_feed.png'), '② 공개 피드 — 실시간 밑줄'),
    (os.path.join(SHOT, 'shot_ul.png'),   '③ 공명 독자 — 같이 멈춘 사람'),
], width_cm=5.3, target_height_cm=9.5)
...
add_image_row([
    (os.path.join(DIAG, 'books_crop.png'), '실제 서비스 — 책 단위 피드 (등록 도서 84권↑ 확대 중)'),
    (os.path.join(SHOT, 'shot_new.png'),   '실제 서비스 — 밑줄 남기기 (사진 1장 → AI 자동 인식)'),
], width_cm=7.8, target_height_cm=11.0)
...
add_image(
    os.path.join(SHOT, 'shot_book.png'),
    width_cm=7.0,
    caption='[그림] 책 상세 화면 — 독자들이 가장 많이 멈춘 페이지 히트맵 + "이 책을 읽는 독자들" 프로파일링 (현재 운영 중인 AI 분석 기능)'
)
...
add_h2('⑫ 성과 목표 및 향후계획')
add_image(
    os.path.join(DIAG, 'diagram_kpi.png'),
    width_cm=15.5,
    caption='[그림 4] 성과 목표 — 현재 대비 6개월 · 12개월 성장 목표 (MAU · 누적 밑줄 · 유료 구독자 · 월 수익)'
)
add_table(
...
# ═══════════════════════════════════════════════════════════
add_h1('[ 이미지 ]')
add_note('아이템의 특징을 나타낼 수 있는 참고사진(이미지)')

add_image_row([
    (os.path.join(SHOT, 'shot_home.png'), '<사진(이미지) 제목> 서비스 첫 화면 — "당신이 멈춘 문장, 누군가도 거기서 멈췄다"'),
    (os.path.join(SHOT, 'shot_ul.png'),   '<사진(이미지) 제목> 밑줄 상세 — 같은 책, 다른 문장에서 멈춘 공명 독자 연결'),
], width_cm=7.6, target_height_cm=12.5)

add_divider()
add_note('마감: 2026년 7월 3일(금) 16시  ｜  신청: www.sbiz24.kr  ｜  사전 필수: edu.sbiz.or.kr AI학습관 강의 1개 수강 완료')

# ─── 저장 ───────────────────────────────────────
out_path = r'd:\dev\underline\grant-application-2026-v5.docx'
doc.save(out_path)
